...
welm_img = open('images/banner-home.jpg', 'rb').read()  
...
class Menu:
  ...
  def login(self):
    ...
    login_choice = input_group('Welcome to the 2024 Seat Allotment Process of Nitte University!', [
    actions('', [ 
  ...
  def menu_for_student(self):
    stud_menu_img = open('images/student-login.jpg', 'rb').read()
    ...
    while(choice!=8): # breaks out of loop when 7 i.e. logout is selected
        if mydata.flag==0: 
            ...
            with use_scope("main", clear=True):
                put_image(stud_menu_img,width='100%',height='300px')
  ...
  def menu_for_admin(self):
    clear('ROOT') 
    admin_menu_img = open('images/admin-login.jpg', 'rb').read()
    mymachine.flag=0
    choice= None 
    while(choice!=7):  # breaks out of loop when 7 i.e. logout is selected
        ...
        if mymachine.flag==0:
            with use_scope("main", clear=True):
                    put_image(admin_menu_img,width='100%',height='300px')
